...
const tableInfo = {
  ...
  cols: [
    `${names.name}-${types.STRING}`,
    `${names.htmlDesc}-${types.TEXTLONG}`,
    `${names.textDesc}-${types.TEXTLONG}`,
    `${names.image}-${types.BLOBmedium}`,
  ],
};

const handleColTypes = (dbType, isSequelize = true) => {
  let result = {};
  const { htmlDesc, textDesc } = names;
